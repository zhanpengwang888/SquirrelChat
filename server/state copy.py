...
class State:
    ...
    def ban_user_helper(self, chatroom, banned_user, connection, current_username):
        ...
        if row_channel[0] is not None and current_username in row_channel[0]:
            ...
            if self.channels[chatroom].ban(self.users[banned_user])\
                ...
                conn.commit()
                conn.close()
                return self.notify(current_username, "You have banned " + banned_user + " from the chat room " +
                                   chatroom + " successfully.", connection)
        else:
            ...
            if row_channel_checking is not None and chatroom in self.channels.keys():
                if current_username in self.channels[chatroom].getUsers().keys():
                    if (self.channels[chatroom].leave(current_username) and current_user_object.leave_chat_room(
                        ...
                        conn.commit()
                        conn.close()
                        return self.notify(current_username, "You have leaved the chat room " + chatroom +
    ...
    def joinChatRoom(self, channel, connection):
        ...
        try:
            ...
            conn = connect_db()
            cur = conn.cursor()
            cur.execute('SELECT user FROM `channel` WHERE channelname=?', (channel,))
            row = cur.fetchone()
            if current_username in self.loggedin_usernames:
                # if channel in self.channels.keys() and row is not None:
                if channel in self.channels.keys() and row[0] is not None: # check if the channel exists or not
                    if current_user_object.join_chat_room(self.channels[channel]) and \
                            self.channels[channel].add_user(current_user_object):
                        if current_username not in row[0]:
                            ...
                            # update the list of users of this channel
                        cur.execute('SELECT channels FROM `user` WHERE username=?', (current_username,))
                        # fetch the list of channels given the user
                        row = cur.fetchone()
                        ...
                        if channel not in row[0]:
                            cur.execute('UPDATE `user` SET channels=? WHERE username=?', (channel_list, current_username))
                            # update the channel list given the username
                        conn.commit()
                        conn.close()
                    ...
                    cur.execute('UPDATE `user` SET channels=? WHERE username=?', (channel_list, current_username))
                    # update the channel list given the username
                    cur.execute('UPDATE `user` SET adminsOfChannels=? WHERE username=?',
                                (channel_list, current_username))
                    # update the channel list which he is the admin given the username
                    conn.commit()
                    conn.close()
                    ...
                    return self.notify(current_username, "You have join the chat room " + channel +
                                       " successfully. You are the admin of the chat room " + channel +
                                       ". ", connection)
        except:
    ...
    def getTopic(self, chatroom, connection):
        ...
        try:
            ...
            if chatroom in self.channels.keys():
                ...
                if row_channel[1] is not None:
                    if current_username in self.channels[chatroom].getUsers().keys() and current_username not in\
                            row_channel[1]:
            ...
            if chatroom in self.channels.keys():
                if current_username in self.channels[chatroom].getUsers().keys():
                    ...
                    if row_channel[0] is not None and current_username in row_channel[0]:
                        ...
                        conn.commit()
                        conn.close()
                        return self.notify(current_username, "You have set the topic of the chat room " + chatroom +
                                           " successfully.", connection)
                    else:
                        return ErrorMessage("You are not the admin of this chat room " + chatroom + ".",
                                            connection).render()
    ...
    def register(self, username, password, connection):
        ...
        if username[0] != "#" and password != "":
            if username not in self.users.keys():
                ...
                conn = connect_db()
                cur = conn.cursor()
                cur.execute('INSERT INTO `user` VALUES(NULL,?,?, NULL, NULL, NULL, NULL, NULL);',
                            (username, password, '', '', '', '', '', ''))
                conn.commit()
                conn.close()
