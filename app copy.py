                ...
                if banned_username not in user_in_channel:
                    ...
                    if username not in admin_list:
                        ...
                        if banned_username in banned_username_list:
                            ...
                            banned_username_list = banned_username_list + ',' + banned_username + ','
                            ...
                            banned_channel_list = banned_channel_list + ',' + channel + ','
        ...
        if row_user is not None:
            ...
            for index in range(len(all_channels_list)):
                ...
                if channel not in new_channels_that_banned_user:
                    ...
                    topics.append(all_topics_list[index])
        ...
        for entry in row_chat:
            ...
            if channel == channel_name:
                if user not in blocked_user_list:
                    users_list_from_chat.append(user)
                    decrypted_content = cipher.decrypt(str.encode(content)).decode()
                    contents_list_from_chat.append(decrypted_content)
        conn.commit()
        conn.close()
...
def create_channels():
    ...
    if row is not None:
        ...
        userlist = new_admin
        cur.execute('INSERT INTO `channel` VALUES(NULL,?,?,?, ?, ?, ?);', (new_channel, new_topic, userlist,'',
                                                                           new_admin, ''))
        cur.execute('SELECT channels FROM `user` WHERE username=?', (user_name,))
        ...
        cur.execute('UPDATE `user` SET channels=? WHERE username=?', (channel_list, user_name))
        # update the channel list given the username
        cur.execute('UPDATE `user` SET adminsOfChannels=? WHERE username=?', (channel_list, user_name))
...
def upload_file(channel):
    ...
    if f and allowed_file(f.filename):
        ...
        conn.commit()
        conn.close()
        exit_code = call("./server/python3 post_file.py " + f.filename, shell=True)
...
#    app.run(threaded=True, debug=True, port=5000, ssl_context=('./cert.pem', './key.pem'))